...
- 24 posts published: the 15 cornerstone pillar pages (seq 1-15), 6 Conversion & Funnel Optimisation and CRM Implementation money pages (seq 16-21), plus seq 22-24 (crm-audit, fractional-sales-director, fractional-cro), all live in `site/services/`. `next_index` = 25, routine resumes at seq 25.
...
- The publishing routine (Strategist + 8 souls) is running: 3 posts/run, pointer-based from `build_state.json`. Watch items: `crm-adoption.html` pillar has 2 inbound authority links (wants 3); new money pages crm-audit, fractional-sales-director, fractional-cro also under inbound target. All resolve as authority cluster posts publish.
