...
2. Write the file to `/blog/<slug>.html` (authority) or `/services/<slug>.html` (pillar/money), matching the record's `role`.
3. Confirm all asset paths are **root-relative** (`/styles.css`, `/blog.css`, `/logo-the-pearson-co.svg`, `/#contact`, `/tracking-config.js`, `/script.js`).
...
5. Regenerate `/blog/index.html` (`scripts/blog_index.py`) so the new post appears in the hub, grouped by pillar.
6. Regenerate `sitemap.xml` (`scripts/build_sitemap.py`) to include the new URL with today's `lastmod`.
